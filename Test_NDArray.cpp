...
namespace
{
	...
	void Test_Div()
	{
		...
		// cube/cube.
		{
			...
			Assert(z.IsEqualTo(NDData::New({2,3,4},
				{
					111      ,56      ,37.6667f,28.5f   ,
					 24.2f   ,20.3333f,17.5714f,15.5f   ,
					 23.4444f,21.2f   ,19.3636f,17.8333f,
					 17      ,15.8571f,14.8667f,14      ,
					 18.2941f,17.3333f,16.4737f,15.7f   ,
					 15.2857f,14.6364f,14.0435f,13.5f
				})),"cube/cube.");
		}
	...
	void Test_Reshape()
	{
...

		{
			...
			print(r1,"r1=");
			Assert(r1.IsEqualTo(NDData::New({1,9},{10,2,3,4,5,6,7,8,9})));		// View, so modified.
			print(r2,"r2=");
			Assert(r2.IsEqualTo(NDData::New({1,9},{1,4,7,2,5,8,3,6,9})));
		}
	}
	...
	void Test_Sum()
	{
		...
		// Sum(-1,true) on matrix.
		{
			NDArray x = NDData::New({4,3},
				{
					-0.7154f, 0.6720f,0.4039f,
					-0.0066f,-0.0338f,1.0195f,
					-1.1144f,-1.9098f,0.4209f,
					 1.0f   , 2.0f   ,3.0f
				});
			print(x);
			NDArray y = x.Sum(-1,true);
		...
		// Sum(0,false) on matrix.
		{
			...
			NDArray y = x.Sum(0,false);
			print(y);
			Assert(y.IsEqualTo(NDData::New({3},{-0.8364f,0.7284f,4.8443f})),"Sum(-1,true) on matrix.");
		}
		...
		// Sum(0,true) on cube.
		{
			NDArray x = NDData::New({2,3,4},
				{
					-0.7154f, 0.6720f,0.4039f,0.114f,
					-0.0066f,-0.0338f,1.0195f,0.124f,
					-1.1144f,-1.9098f,1.4209f,0.134f,

					-0.7154f, 0.6720f,0.4039f,0.214f,
					-0.0066f,-0.0338f,1.0195f,0.224f,
					-2.1144f,-2.9098f,2.4209f,0.234f
				});
			print(x);
			NDArray y = x.Sum(0,true);
			print(y);
			Assert(y.IsEqualTo(NDData::New({1,3,4},
				{
					-1.4308f,  1.3440f,  0.8078f,  0.3280f,
					-0.0132f, -0.0676f,  2.0390f,  0.3480f,
					-3.2288f, -4.8196f,  3.8418f,  0.3680f
				})),"Sum(0,true) on cube.");		
		}
		...
		// Sum(1,true) on cube.
		{
			...
			Assert(y.IsEqualTo(NDData::New({2,1,4},
				{
					-1.8364f,-1.2716f,2.8443f,0.372f,
					-2.8364f,-2.2716f,3.8443f,0.672f
				})),"Sum(1,true) on cube.");		
		}
		...
		// Sum(2,true) on cube.
		{
			...
			Assert(y.IsEqualTo(NDData::New({2,3,1},
				{
					0.4745f,1.1031f,-1.4693f,
					0.5745f,1.2031f,-2.3693f
				})),"Sum(2,true) on cube.");		
		}
...
void Test_NDIterator2()
{
	// New(shape)
	{
		NDArray x = NDData::Zeros({32,8,128});
		Assert(x.Shape()==NDShape({32,8,128}),"New(shape): Shape.");
		Assert(x.Size()==32768,"New(shape): Vector size");
		x.Exp();
	}
}
...
void Test_NDArray()
{
	...
	// New(shape)
	{
		...
		Assert(x.Size()==6,"New(shape): Vector size");
	}
	...
	// New(shape,value)
	{
		NDArray x = NDData::New({3,2},1.23f);
		Assert(x.IsEqualTo(NDData::New({3,2},
			{
				1.23f,1.23f,
				1.23f,1.23f,
	...
	// _ClipNorm
	{
		...
		Assert(x.IsEqualTo(NDData::New({4,3},
			{
				10.837043964422636f,20.688902113897761f,30.540760263372885f,
				40.392618412848009f,11.822229779370149f,21.674087928845271f,
				31.525946078320395f,41.377804227795522f,12.807415594317661f,
				23.644459558740298f,32.511131893267908f,42.362990042743029f
			})),"_ClipNorm[].");
	}
	...
	// Entropy.
	{
		...
		Assert(y.IsEqualTo(NDData::New({2},{1.2798542258336676f,0.99727090215780811f})),"Entropy: Values.");
	}

	// Exp.
	{
		NDArray x = NDData::New({3,3},{0.11f,0.12f,0.13f,0.21f,0.22f,0.23f,0.31f,0.32f,0.33f});
		NDArray y = x.Exp();
		Assert(y.IsEqualTo(NDData::New({3,3},
			{
				1.1162780704588713f,1.1274968515793757f,1.1388283833246218f,
				1.2336780599567432f,1.2460767305873808f,1.2586000099294778f,
				1.3634251141321778f,1.3771277643359572f,1.3909681284637803f
			})),"Exp.");
	}
